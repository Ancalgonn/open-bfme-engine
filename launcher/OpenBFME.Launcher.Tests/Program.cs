...
static Task TestOptions()
{
    ...
    var options = LauncherOptions.Parse(new[]
    {
        ...
        "--import-bfme2", "--bfme2-path", @"F:\BFME2"
    });
    Check(options.ImportGame == "bfme2", "flags not parsed");
    Check(options.ManifestUri is null, "non-stable channels require an explicit immutable manifest");
...
{
    ReleaseUriPolicy.Validate(new Uri("https://github.com/Ancalgonn/open-bfme-engine/releases/download/v1/game.zip"));
    ...
    Throws<InvalidOperationException>(() => ReleaseUriPolicy.Validate(new Uri("http://github.com/Ancalgonn/open-bfme-engine/a")));
    ...
    Throws<InvalidOperationException>(() => ReleaseUriPolicy.Validate(
        new Uri("https://github.com/evil/repo/Ancalgonn/payload")));
    Throws<InvalidOperationException>(() => ReleaseUriPolicy.Validate(
...
static Task TestManifest()
{
    var json = """
    {
      ...
      "schemaVersion":1,
      "repository":"Ancalgonn/open-bfme-engine",
      "version":"0.1.0-alpha.1",
      "channel":"playtest",
      "commit":"0123456789abcdef0123456789abcdef01234567",
      "packages":[{
        "name":"game.zip",
        "url":"https://github.com/Ancalgonn/open-bfme-engine/releases/download/v0.1.0-alpha.1/game.zip",
        ...
        "name":"launcher.zip",
        "url":"https://github.com/Ancalgonn/open-bfme-engine/releases/download/v0.1.0-alpha.1/launcher.zip",
...
static Task TestRedaction()
{
    var method = typeof(ImporterRunner).GetMethod("Redact",
        System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static)!;
    var value = (string)method.Invoke(null, new object[] { @"failed at F:\BFME2\data\asset.big" })!;
    Check(!value.Contains("F:\\BFME2", StringComparison.OrdinalIgnoreCase), "private path leaked");
    return Task.CompletedTask;
}
...
static TestPackageSet ManifestFor(byte[] bytes, string version, string commit)
{
    ...
    var manifest = new ReleaseManifest(ReleaseManifest.ExpectedSchema, 1,
        "Ancalgonn/open-bfme-engine", version, "playtest", commit,
        new[]
        {
            new ReleasePackage("game.zip",
                $"https://github.com/Ancalgonn/open-bfme-engine/releases/download/v{version}/game.zip",
                sha, bytes.Length, ExpandedSize(bytes), "game-windows-x64"),
            new ReleasePackage("launcher.zip",
                $"https://github.com/Ancalgonn/open-bfme-engine/releases/download/v{version}/launcher.zip",
