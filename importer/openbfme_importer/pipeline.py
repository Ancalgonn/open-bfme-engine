...
# Exact resolved-source contract for the current 83-rule men-fords-v0 profile.
MEN_FORDS_SOURCE_ENTRY_COUNT = 335
...
def _importer_recipe_report() -> dict[str, Any]:
    ...
    release_identity_path = root / "release-identity.json"
    candidates = [
        *sorted((root / "importer" / "openbfme_importer").rglob("*.py")),
        *sorted((root / "importer" / "blender").rglob("*.py")),
        root / "importer" / (
            "requirements-release-win.txt"
            if (root / "importer" / "requirements-release-win.txt").is_file()
            else "requirements-win.txt"
        ),
    ...
    for path in sorted(
        ...
        digest.update(b"\n")
    if release_identity_path.is_file():
        identity = read_json(release_identity_path)
        if not isinstance(identity, dict):
            raise RuntimeError("bundled importer release identity is invalid")
        candidate_commit = identity.get("commit")
        candidate_clean = identity.get("sourceClean")
        if (
            identity.get("schema") != RELEASE_IDENTITY_SCHEMA
            or identity.get("schemaVersion") != 1
            or not _is_git_commit(candidate_commit)
            or candidate_clean is not True
        ):
            raise RuntimeError("bundled importer release identity is invalid")
...
class ImportPipeline:
    ...
    def _ffmpeg_executable(self) -> Path | None:
        """Resolve FFmpeg from this import state's pinned tools before PATH."""

        configured = os.environ.get("OPENBFME_FFMPEG", "").strip()
        ...
        return discover_executable("ffmpeg", "OPENBFME_FFMPEG")
    ...
    def _canonical_tool_report(self) -> dict[str, Any]:
        ...
        if ffmpeg:
            inspected = inspect_tool("ffmpeg", executable=ffmpeg)
            report["ffmpeg"] = {
    ...
    def _end_w3d_conversion_batch(self) -> None:
        ...
        # Archive-provisioned plugins intentionally have no Git metadata.
        # prepare_opensage_plugin_checkout() has already re-hashed that entire
        # portable tree above, which is the equivalent cleanliness proof.
        plugin_clean = (
            git_worktree_clean(plugin) if (plugin / ".git").exists() else True
        )
        if not plugin_clean:
...
def _audit_recipe(
    recipe: Any, errors: list[str], *, require_clean: bool = False
) -> str:
    ...
    elif require_clean and clean is not True:
        errors.append("retail provenance importer recipe is not from a clean release source")
    return str(declared or "")
...
def _audit_tool_attestations(tools: Any, profile: str, errors: list[str]) -> int:
    ...
    from .bootstrap import (
        ...
        PYTHON_VERSION,
        python_runtime_identity_is_pinned,
    )
    ...
    if (
        ...
        or not python_runtime_identity_is_pinned(python)
...
def _audit_retail_provenance(
    ...
    summary["importer_recipe_sha256"] = _audit_recipe(
        manifest.get("importer_recipe"),
        errors,
        require_clean=profile == "men-fords-v0",
    )
