...
def initialize_w3d_converter(plugin_root: Path) -> None:
    ...
    # The third-party add-on updater writes status beside its source by
    # default. Keep the hash-pinned plugin immutable and avoid Windows path
    # length failures by redirecting that disposable state to the OS temp
    # directory before registration.
    updater = io_mesh_w3d.addon_updater_ops.updater
    updater.stage_path = tempfile.mkdtemp(prefix="openbfme-w3d-updater-")
